 ...
 * be used to identify the shape. this is done by calling -<br>
 * <b><pre>pickShape(PApplet papplet, int x, int y) </pre></b><br>
 * 
 * The first time the setTexure(...) method is called with a particular filename will result
 * in the image being loaded and cached. Subsequent calls with the saved filename will use
 * the cached image. This means that only one image per file exists in RAM.
 * 
 * @author Peter Lager
 *
 */
public abstract class Shape3D implements PConstants, SConstants {
	...
	protected static int OPTIMIZED_STROKE = ENABLE_OPTIMIZED_STROKE;
	...
	/**
	 * By default Processing optimizes stroke drawing to reduce execution time. Unfortunately
	 * this can cause visual artifacts to appear to avoid these set this to false to disable 
	 * stroke optimization (but at the expanse of speed).
	 * 
	 * There is no impact if stroke darwing mode is unused.
	 * 
	 * @param on false to avoid visual artufacts
	 */
	public static void setOptimizedStrokeOn(boolean on){
		if(on)
			OPTIMIZED_STROKE = ENABLE_OPTIMIZED_STROKE;
		else
			OPTIMIZED_STROKE = DISABLE_OPTIMIZED_STROKE;
	}
	
	public static boolean isOptimizedStrokeOn(){
		return OPTIMIZED_STROKE == ENABLE_OPTIMIZED_STROKE;
	}
	 ...
	 * It does this by using a P3D off-screen buffer, copying the 
	 * transformation matrices from the PApplet then draws each 
	 * shape with a unique colour. The colour at position x,y then 
	 * identifies the 3D shape displayed at that position. <br>
	 * 
	 * Since the buffer must copy the current state of the 
	 * transformation matrices it is best that this is called from 
	 * the draw method. <br>
	 * 
	 * IMPORTANT: the very first call to this method will return null.
	 * 
	...
	// Next line added to make copy of buffer available when debugging
	public static PImage img = null;
	...
	/**
	 * Change the world position and up direction for a new centre of rotation. <br>
	 ...
	 */
	public static PGraphicsOpenGL getPickBuffer(){
		return pickBuffer;
	}
	 ...
	 * @param nRptNS
	 */
	public void setTexture(String fname, float nRptEW, float nRptNS){ }

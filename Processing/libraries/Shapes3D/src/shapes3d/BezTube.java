...
public class BezTube extends Mesh2DCoreWithCaps {
	 ...
	 */
	public BezTube(PApplet app, P_Bezier3D bez, float rad, int nbrSlices, int nbrSegments){
		super();
	 ...
	 * @param nbrSegments number of sections around tube
	 */
	public BezTube(PApplet app, P_Bezier3D bez, I_RadiusGen radGen, int nbrSlices, int nbrSegments){
		super();
	 ...
	 * @param centreOfRotation new centre of orientation
	 */
	public BezTube(PApplet app, P_Bezier3D bez, I_RadiusGen rad, int nbrSlices, int nbrSegments, 
			PVector orientation, PVector centreOfRotation){
		super();
	 ...
	 * @param nbrSegments number of sections around tube
	 */
	private void ctorCore(PApplet app, P_Bezier3D bez, I_RadiusGen radGen, int nbrSlices, int nbrSegments){
		this.app = app;
		nsPieces = nbrSlices;
	...
	public void shapeOrientation(PVector up, PVector centreOfRot){
		...
		if(changed) {
			...
			setBez(new P_Bezier3D(ctlPoint, ctlPoint.length));
		}
		calcXYZ();
	}
	...
	protected void calcShape(){
		...
		startEC = new EndCapDisc(this);
		endEC = new EndCapDisc(this);
	}
	 ...
	 */
	public void setDetails(P_Bezier3D bez, float radius){
	 ...
	 */
	public void setDetails(P_Bezier3D bez, I_RadiusGen rad){
		while(drawLock){
			try {
				Thread.sleep(10);
	 ...
	 * @param bez
	 */
	public void setBez(P_Bezier3D bez){
		while(drawLock){
			try {
				Thread.sleep(10);
			} catch (InterruptedException e) {}

...
#include "cbuf.h"
#include "include/pfodClientContext.h"
#include "c_types.h"
...
size_t WiFiClient::write(const uint8_t *buf, size_t size) {
  if (!_client || !size) {
#ifdef DEBUG
   DEBUG_OUTPUT.println("no client or nothing to send return 0");
#endif   
    return 0;
  }
  size_t bytesWritten =  _client->write(reinterpret_cast<const char*>(buf), size);
#ifdef DEBUG
   DEBUG_OUTPUT.print("bytesWritten = ");
   DEBUG_OUTPUT.println(bytesWritten);
#endif   
  if (bytesWritten == ((size_t) - 1)) {
    // error abort connection
    if (!_client) {
#ifdef DEBUG
   DEBUG_OUTPUT.print(" error no client return 0");
#endif   
      return 0;
    }
    _client->abort();
